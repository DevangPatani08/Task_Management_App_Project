        ...
        <>
            <div className={`min-h-56 flex flex-col gap-0 border rounded-md p-4 mb-3 ${task.completed ? 'bg-neutral-50 border-neutral-300' : overdue ? 'bg-white border-red-300' : 'bg-white border-neutral-300'}`}>
                <div className='flex items-center justify-between gap-2'>
                    ...
                    {
                        (toggling.load === true && toggling.id === task._id) ? <div className='animate-spin rounded-full h-5 w-5 border-t-4 border-primary-500' />
                        :
                ...
                <div className='flex items-center justify-between gap-2'>
                    ...
                    <div className='flex items-center justify-end gap-2'>
                        <Button type='button' btnType='iconsOnlySecondary' handleClick={() => setShowMessageModal(true)} data-tooltip-id="my-tooltip" data-tooltip-content="View Message" data-tooltip-place="bottom">
                            <BadgeInfo className='w-4.5 h-4.5' />
                        </Button>
                        <Button type='button' btnType='iconsOnlySecondary' handleClick={() => onEdit(task)} disabled={task.completed} data-tooltip-id="my-tooltip" data-tooltip-content={`${task.completed ? 'Cannot edit completed task' : 'Edit task'}`} data-tooltip-place="bottom">
                            <Edit className='w-4.5 h-4.5' />
                        </Button>
                        <Button type='button' btnType='iconsOnlySecondary' handleClick={() => setShowDeleteModal(true)} data-tooltip-id="my-tooltip" data-tooltip-content="Delete task" data-tooltip-place="bottom">
